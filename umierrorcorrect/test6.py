...
def cluster():
    bamfilename='/medstore/Illumina_Tobias/twist/DNA10_S4/DNA10_S4.sorted.bam'    
    #bamfilename='/medstore/Illumina_Tobias/juli/runuec3/EGFR-T790-Single-tube-Roche-20190108-A1_S2/EGFR-T790-Single-tube-Roche-20190108-A1_S2_L001_R1_001.sorted.bam'
    position_threshold=20
    regions, ends = readBam(bamfilename, position_threshold)
    #print(regions['4'].keys())
    
    umi_dict=regions['4'][55564240]
...
def consensus_test(umis,bamfilename):
    start=time.time()
    u2={'GGCTAC': umis['GGCTAC']}
    position_matrix, singleton_matrix = get_cons_dict(bamfilename, umis, '4', 55564240, 55564854, True)
    seqs=position_matrix['GGCTAC']
    with pysam.AlignmentFile(bamfilename,'rb') as f, pysam.AlignmentFile('test_out.bam','wb',template=f) as g:
        for s in seqs:
            g.write(s)
    #seqs=position_matrix['GGAGAC']
    #with pysam.AlignmentFile(bamfilename,'rb') as f, pysam.AlignmentFile('test_out2.bam','wb',template=f) as g:
    #    for s in seqs:
    #        g.write(s)
    return(seqs)
    ...
    #umis_sel=umis
    #with open('position_matrix2.pickle','wb') as f:
    #        pickle.dump(position_matrix,f)
    #consensuses={}
    #for umi in position_matrix:
...
def main():
    ...
    seqs=consensus_test(umis,bamfilename)
    cons=getConsensus2(seqs,'4',1950,0.6,umis)
    print(cons)
    #with pysam.AlignmentFile(bamfilename,'rb') as f, pysam.AlignmentFile('test_out.bam','wb',template=f) as g:
    #    for s in seqs:
    #        g.write(s)
    #print(cons)
    #with open('consensus_matrix2.pickle','wb') as f:
    #    pickle.dump(cons,f)
    #position_matrix, singleton_matrix = get_cons_dict(bamfilename, umis, '3', 178952031, 178952088, True)
    ...
    #end=time.time()
    #print(end-start)
    #print(consensuses['AAGGGTTAGCGG'].seq)
    #print(umis)
