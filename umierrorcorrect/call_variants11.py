...
def parseArgs():
    ...
    parser.add_argument('-a', '--all', dest='use_all_positions', action='store_true', help='Include this flag if all positions (e.g. positions without annotation) should be considered in variant calling.')
...
def parse_cons_file(filename,fsize=3, use_all_positions=False):
    n1=[]
    f1=[]
    c1=[]
    data=[]
    with open(filename) as f:
        ...
        for line in f:
            ...
            name=parts[3]
            #print(name)
            if name not in "" or use_all_positions:
                famsize=parts[-4]
...
def run_call_variants(args):
    ...
    f1,n1,a1,data=parse_cons_file(args.cons_file, args.fsize, args.use_all_positions)
    f1 = np.array(f1)
    n1 = np.array(n1)
    a1 = np.array(a1)
    data = np.array(data)
