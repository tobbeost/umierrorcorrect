...
from preprocess8 import run_preprocessing
from run_mapping2 import run_mapping
from umi_error_correct7 import run_umi_errorcorrect
import argparse
import os
import logging
...
def parseArgs():
    ...
    group1.add_argument('-o', '--output_path', dest='output_path', help='Path to the output directory, required', required=True)
    ...
    group2.add_argument('-sl', '--spacer_length', dest='spacer_length', help='Length of spacer (The number of nucleotides between the UMI and the beginning of the read). The UMI + spacer will be trimmed off, and the spacer will be discarded. Default=%(default)s', default='0')
    group2.add_argument('-mode', '--mode', dest='mode', help="Name of library prep, Either 'single' or 'paired', for single end or paired end data respectively, [default = %(default)s]", default="paired")
    group2.add_argument('-dual', '--dual_index', dest='dual_index', help='Include this flag if dual indices are used (UMIs both on R1 and R2)', action='store_true')
    group2.add_argument('-reverse', '--reverse_index', dest='reverse_index', help="Include this flag if a single index (UMI) is used, but the UMI is located on R2 (reverse read). Default is UMI on R1.", action='store_true')
    
    group3 = parser.add_argument_group('UMI clustering options')
    group3.add_argument('-regions_from_bed', dest='regions_from_bed', help='Include this flag if regions used for UMI clustering and variant calling should be defined from the BED file. Default is to detect the regions automatically from the BAM file. ', action='store_true')
    group3.add_argument('-d', '--edit_distance', dest='edit_distance_threshold', help="Edit distance threshold for UMI clustering, [default = %(default)s]", default=1)
    ...
    group5.add_argument('-tmpdir', '--tmp_dir', dest='tmpdir', help="temp directory where the temporary files are written and then removed. Should be the scratch directory on the node. Default is a temp directory in the output folder.")
    group5.add_argument('-t', '--num_threads', dest='num_threads', help='Number of threads to run the program on. Default=%(default)s', default='1')
