    ...
    for contig in regions:
        ...
        for current_index, start in enumerate(starts):
            if current_end + pos_threshold < start:
                newregions[contig].append((current_start - pos_threshold, current_end + pos_threshold, ','.join(current_name)))
                current_start = start
                current_name=[]
        ...
        newregions[contig].append((current_start - pos_threshold, current_end + pos_threshold, ','.join(current_name))) #save the last entry
    return(newregions)
